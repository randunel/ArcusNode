  ...
  var _handleMessage = function(nc, message, remoteInfo, pkt, socket) {
    ...
    switch(message.type){
      ...
      case RTMFP.COMMAND:
        ...
        if(typeof nc.commands.get(message.commandName) === 'object') {
          //TODO: find a better way to pass the context, maybe wrap the function in an object
          //__p.commands.get(message.commandName).method.call(__p.commands.get(message.commandName).context, nc, message, _self, _self.commandResult);
          nc.commands.get(message.commandName).method.call(nc.commands.get(message.commandName).context, message, nc.commandResult);
        } else {
        ...
        //nc.emit(message.commandName, message.commandData);
